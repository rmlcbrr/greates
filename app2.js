                ...
                if(state == document.getElementById("content-capture")){ 
                    ...
                    vDiv.appendChild(image); 
                        console.log("Activated");
                         ...
                         var img_data=image.src;
                         var fd = new FormData();
                         fd.append('img_data',img_data);
                         $.ajax({
                        url:"../../java/generate.php",
                        data: fd,
                        processData: false,
                        contentType: false,
                        type:'POST',
                        ...
                        success:function(result){   
                            var arr=[];
                            arr=JSON.parse($.trim(result));
                            console.log($.trim(result));
                            var output_result=arr[0];
                            output_result=output_result.toString();
                                if($.trim(output_result)==="Found" ||  $.trim(output_result)==='Found'    ||  $.trim(output_result)=='Found' || $.trim(output_result)=="Found"){
                                     $.notify("RECOGNIZE BIOMETRIC!!","success");    
//*******************************************LOGIN
/*
                            var fd = new FormData();    
                            fd.append('uname', arr[1]);
                            fd.append('status',"9");
                            $.ajax({
                             ...
                             $.notify(" Incorrect Credential Used   or Account is deactivated!!! ","warning"); 
                             return false;
                             }
                                }, 2000);
                              }
                           ...
                           }else if($.trim(output_result)==="Not_Found" ||  $.trim(output_result)==='Not_Found'    ||  $.trim(output_result)=='Not_Found' || $.trim(output_result)=="Not_Found"){
                                $.notify("Unrecognized biometric record!!","warning");          
                                    }
