...
//************************************STEP3**********************************//
  ...
  $('#dones').on('click', function() {
            var color_blind_exam = [];
            $.each($("input[name='color_blind_exam']:checked"), function(){
                color_blind_exam.push($(this).val());
            });

  var  corrective_lens_right= [];
               $.each($("input[name='corrective_lens_right']:checked"), function(){
                corrective_lens_right.push($(this).val());
            });

  var  corrective_lens_left= [];
               $.each($("input[name='corrective_lens_left']:checked"), function(){
                corrective_lens_left.push($(this).val());
            });


                  var visual_act_right =$("input[name='visual_act_right']:checked").val();

                  var visual_act_left =$("input[name='visual_act_left']:checked").val();


  var  hearingaid= [];
               $.each($("input[name='hearingaid']:checked"), function(){
                hearingaid.push($(this).val());
            });
   window.location.replace("results?eyecolor="+encodeURI(color_blind_exam.join(","))+'&correctiver='+encodeURI(corrective_lens_right)+'&correctivel='+encodeURI(corrective_lens_left)+'&visual_act_right='+encodeURI(visual_act_right )+'&visual_act_left='+encodeURI(visual_act_left )+'&hearingaid='+encodeURI(hearingaid));
      return false;
    });   
