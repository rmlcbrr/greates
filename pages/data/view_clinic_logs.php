...
$sql = "SELECT count(*) as totals from medical_record	as	mr	INNER	JOIN	tbl_clinics	as	tc	ON	tc.id=mr.branch_id WHERE 1=1	";
...
$sql = "SELECT mr.*,tc.name from medical_record	as	mr	INNER	JOIN	tbl_clinics	as	tc	ON	tc.id=mr.branch_id WHERE 1=1	";
...
//if(($unserialized['profile_position'])=="Driver" && ($unserialized['profile_position'])!=""){
	...
	<button type='button' id='".base64_encode($row["uid"])."'   class='  btn-success btn-sm view_btn' data-toggle='modal' title='View Record'><i class='fa fa-eye' ></i></button>";
	$data[] = $nestedData;
			//	}
}
