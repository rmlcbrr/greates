...
//id, activity, date, uid, type, remarks
$sql = "SELECT * from tpl_sys_log ";
...
$sql = "SELECT * from tpl_sys_log WHERE 1=1 ";

if( !empty($requestData['search']['value']) ) {   // if there is a search parameter, $requestData['search']['value'] contains search parameter
     $sql.=" AND (activity LIKE '%".$requestData['search']['value']."%' ";
...
//if(($unserialized['profile_position'])=="Driver" && ($unserialized['profile_position'])!=""){
     ...
     $nestedData[] = htmlentities(htmlspecialchars($row["activity"]));
     $nestedData[] = htmlentities(htmlspecialchars(($row['remarks'])));
     $nestedData[] = htmlentities(htmlspecialchars(($row['type'])));
     $nestedData[] = htmlentities(htmlspecialchars(($row['date'])));
